...
import {
  ...
  verifyUserTokenGraceful,
} from "@/store/user/slices/userAuthSlice";
  ...
  const handleSendMessage = async (message: string, model: ModelType) => {
    ...
    try {
      ...
      const payload: any = {
        ...
        ...(!currentChatId &&
          !isUserLoggedIn && { chat_title: generateChatTitle(message) }),
        ...(!currentChatId &&
          isUserLoggedIn && { chat_title: generateChatTitle(message) }),
      };
      ...
      if (!response.ok && response.status === 401 && isUserLoggedIn) {
        ...
        if (companyInfo) {
          ...
          if (tokenResponse.ok) {
            ...
            const retryResponse = await fetch(
              getApiUrl(`/public/chatbot/${slug}/chat`),
              {
                method: "POST",
                headers: {
                  "Content-Type": "application/json",
        ...
        throw new Error(
          "Authentication failed and could not create guest session",
        );
      }
